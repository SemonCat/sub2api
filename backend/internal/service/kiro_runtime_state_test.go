...
func TestExecuteKiroUpstreamAutoIDCFallsBackAcrossAuthSurfacesOn403(t *testing.T) {
	...
	codeWhispererBody, err := io.ReadAll(upstream.requests[0].Body)
	require.NoError(t, err)
	require.Contains(t, string(codeWhispererBody), `"profileArn":"arn:aws:codewhisperer:us-east-1:123456789012:profile/AUTO"`)
}
